...
public class Compress {
	...
	public LinkedList<hfm> list = new LinkedList<hfm>();
	//统计次数
	...
	//构造哈夫曼树
	public hfm createTree(){
		//将次数作为权值构造森林
		for (int i = 0; i < times.length; i++) {
			if(times[i]!=0){
				hfm node = new hfm(times[i],i);
				//将构造好的节点加入到容器中的正确位置
		...
		while(list.size()>1) {
			//获取容器中第一个元素
			hfm firstNode =list.removeFirst();
			//获取中新的第二个元素
			hfm secondNode =list.removeFirst();
			//将权值最小的两个节点构造成父节点
			hfm fatherNode =
					new hfm(firstNode.getData()+secondNode.getData(),-1);
	...
	//利用前序遍历获取编码表
	public void getHuffmCode(hfm root,String code){
		//往左走，哈夫曼编码加0
